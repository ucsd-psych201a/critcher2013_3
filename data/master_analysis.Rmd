...
cleaned_data <- dat %>%
  ...
  rowwise() %>%
  mutate(
    justin_variance = var(c(Justin_Q2, Justin_Q3, Justin_Q4), na.rm = TRUE), # Variance for Justin
    nate_variance = var(c(Nate_Q2, Nate_Q3, Nate_Q4), na.rm = TRUE)          # Variance for Nate
  ) %>%
  ungroup() %>%
...
Groups the data by condition and provide a clear comparison of how the participants perceived Justin's decision speed and Nate's decision speed across the two conditions. (Note: the mean should be different between Justin and Nate but it should be similar across condition).

Then we will determine whether if there is a statistically significant difference in the mean scores of Justin's speed and Nate's speed.
...
character_eval_combined_long <- character_eval_combined %>%
  pivot_longer(
    ...
    names_to = "person",                           # Create a column for person
    values_to = "score"                            # Create a column for scores
  ) %>%
  mutate(
    person = ifelse(person == "quick_justin_score", "Justin", "Nate") # Relabel person column
  )  %>%
  select(Participant_ID, condition, person, score) # Keep only essential columns
...
```{r}
anova_combined <- aov(score ~ condition * person + Error(Participant_ID/person), data = character_eval_combined_long)
...
posthoc_results <- emmeans(anova_combined, pairwise ~ condition * person)
...
anova_combined_lmer <- lmer(
  score ~ condition * person + (1 | Participant_ID),
...
```{r}
anova_group3 <- aov(score ~ condition * person + Error(Participant_ID/person), data = character_eval_group3_long)
...
```{r}
# Conduct post hoc pairwise comparisons using estimated marginal means
posthoc_group3_results <- emmeans(anova_group3, pairwise ~ condition * person)
